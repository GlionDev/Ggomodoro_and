...
/**
 * 히스토리 화면의 비즈니스 로직과 데이터 상태를 관리하는 ViewModel입니다. 저장된 세션 목록을 로드하고 메모 업데이트 및 세션 삭제 기능을 제공합니다.
 *
...
constructor(
    ...
    val historyState: StateFlow<List<TimerSession>> =
            getHistoryUseCase()
                    .stateIn(
    ...
    fun updateMemo(session: TimerSession, newMemo: String) {
        viewModelScope.launch { updateMemoUseCase(session.id, newMemo) }
    }
    ...
    fun deleteSession(session: TimerSession) {
        viewModelScope.launch { deleteSessionUseCase(session.id) }
    }
}
